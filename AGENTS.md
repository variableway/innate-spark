...
| `tools/innate-spark-cli/` | The only code allowed in this repo: `skill-spark` multi-subcommand CLI (Bun, built to `tools/innate-spark-cli/dist/`, gitignored) |
...
- Record implementation locations only in each product’s `links.md`. If the task is to write code, do it in the implementation project (or ask to open / attach that folder). Do not add application source, lockfiles, or `node_modules` here — except `tools/innate-spark-cli/`, and shared base projects under `base/`.
...
$SPARK selfhost profiles --config tools/innate-spark-cli/config.json   # list SMB hosts from config.json
$SPARK selfhost open --profile lazycat --config tools/innate-spark-cli/config.json

# Test + build the single binary (→ tools/innate-spark-cli/dist/skill-spark, gitignored)
